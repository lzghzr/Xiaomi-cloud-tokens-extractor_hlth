...
class XiaomiCloudConnector(ABC):
    ...
    def get_devices(self, country):
        ...
        params = {"data": '{"page_size":50,"status":1}'}
    ...
    def get_api_url(country):
        return "https://" + ("" if country == "cn" else (country + ".")) + "hlth.io.mi.com/app"
...
class PasswordXiaomiCloudConnector(XiaomiCloudConnector):
    ...
    def do_2fa_email_flow(self, notification_url: str) -> bool:
        ...
        if not sts_url and r.text:
            idx = r.text.find("https://sts-hlth.api.io.mi.com/sts")
            if idx != -1:
                end = r.text.find('"', idx)
                if end == -1:
                    end = idx + 300
        ...
        self._serviceToken = self._session.cookies.get("serviceToken", domain=".sts-hlth.api.io.mi.com")
        ...
        self.userId = self.userId or self._session.cookies.get("userId", domain=".xiaomi.com") or self._session.cookies.get("userId", domain=".sts-hlth.api.io.mi.com")
        self._cUserId = self._cUserId or self._session.cookies.get("cUserId", domain=".xiaomi.com") or self._session.cookies.get("cUserId", domain=".sts-hlth.api.io.mi.com")
        return True

    def install_service_token_cookies(self, token: str):
        for d in [".hlth.io.mi.com", ".io.mi.com", ".mi.com"]:
...
def main() -> None:
    ...
    if logged:
        ...
        for current_server in servers_to_check:
            ...
            if devices is not None and devices["result"]["list"] is not None and len(devices["result"]["list"]) > 0:
                print_if_interactive(f'Devices found for server "{current_server}":')
                for device in devices["result"]["list"]:
                    if device["detail"] is not None:
                        ...
                        if "did" in device:
                            print_entry("ID", device["sid"], 3)
                        if "model" in device:
                            print_entry("MODEL", device["model"], 3)
                        if "mac" in device["detail"]:
                            print_entry("MAC", device["detail"]["mac"], 3)
                        if "beaconkey" in device["detail"]:
                            print_entry("BLE KEY", device["detail"]["beaconkey"], 3)
                        if "token" in device["detail"]:
                            print_entry("TOKEN", device["detail"]["token"], 3)
                        if "irq_key" in device["detail"]:
                            print_entry("IRK", device["detail"]["irq_key"], 3)
                    print_tabbed(f"{Fore.BLUE}---------", 3)
                    print_if_interactive()
    else:
